...
#define HOPSLIMIT 27
#define TIMELOCKLIMIT 2023+FINALTIMELOCK
...
// 0 default 1 capacity 2 fee 3 time
#define COMPAREMETHOD 3
...
// Kept this original one as all algorithm has the same init method
...
void *max_capacity_flow_thread(void *arg) {
    struct payment *payment;
    struct array *hops;
    void *data;
    long payment_id;
    ...
    while (1) {
        ...
        payment = array_get(thread_args->payments, payment_id);
        pthread_mutex_unlock(&data_mutex);
        hops = max_capacity_flow(payment->sender, payment->receiver, payment->amount, thread_args->network,
                                 thread_args->current_time, thread_args->data_index, &error);
        paths[payment->id] = hops;
    }
...
void *minimum_fee_flow_thread(void *arg) {
    struct payment *payment;
    struct array *hops;
    void *data;
    long payment_id;
    ...
    while (1) {
        ...
        payment = array_get(thread_args->payments, payment_id);
        pthread_mutex_unlock(&data_mutex);
        hops = minimum_fee_flow(payment->sender, payment->receiver, payment->amount, thread_args->network,
                                thread_args->current_time, thread_args->data_index, &error);
        paths[payment->id] = hops;
    }
...
void *yen_thread(void *arg) {
    ...
    while (1) {
        ...
        payment = array_get(thread_args->payments, payment_id);
        pthread_mutex_unlock(&data_mutex);
        hops = yen(payment->sender, payment->receiver, payment->amount, thread_args->network,
                   thread_args->current_time, thread_args->data_index, &error);
        paths[payment->id] = hops;
    }
...
void *landmark_thread(void *arg) {
    ...
    while (1) {
        ...
        payment = array_get(thread_args->payments, payment_id);
        pthread_mutex_unlock(&data_mutex);
        hops = landmark(payment->sender, payment->receiver, payment->amount, thread_args->network,
                        thread_args->current_time, thread_args->data_index, &error);
        paths[payment->id] = hops;
    }
...
void run_max_capacity_flow_threads(struct network *network, struct array *payments, uint64_t current_time) {
    long i;
    pthread_t tid[N_THREADS];
    struct thread_args *thread_args;
    ...
    for (i = 0; i < N_THREADS; i++) {
        ...
        thread_args->current_time = current_time;
        thread_args->data_index = i;
        pthread_create(&(tid[i]), NULL, max_capacity_flow_thread, (void *) thread_args);
    }
...
void run_minimum_fee_flow_threads(struct network *network, struct array *payments, uint64_t current_time) {
    long i;
    pthread_t tid[N_THREADS];
    struct thread_args *thread_args;
    ...
    for (i = 0; i < N_THREADS; i++) {
        ...
        thread_args->current_time = current_time;
        thread_args->data_index = i;
        pthread_create(&(tid[i]), NULL, minimum_fee_flow_thread, (void *) thread_args);
    }
...
void run_landmark_threads(struct network *network, struct array *payments, uint64_t current_time) {
    long i;
    pthread_t tid[N_THREADS];
    struct thread_args *thread_args;
    ...
    for (i = 0; i < N_THREADS; i++) {
        ...
        thread_args->current_time = current_time;
        thread_args->data_index = i;
        pthread_create(&(tid[i]), NULL, landmark_thread, (void *) thread_args);
    }
...
void run_yen_threads(struct network *network, struct array *payments, uint64_t current_time) {
    long i;
    pthread_t tid[N_THREADS];
    struct thread_args *thread_args;
    ...
    for (i = 0; i < N_THREADS; i++) {
        ...
        thread_args->current_time = current_time;
        thread_args->data_index = i;
        pthread_create(&(tid[i]), NULL, yen_thread, (void *) thread_args);
    }
...
double calculate_probability(struct element *node_results, long to_node_id, uint64_t amount, double node_probability,
...
double get_probability(long from_node_id, long to_node_id, uint64_t amount, long sender_id, uint64_t current_time,
...
// Compare distance by capacity for max flow
int compare_distance_by_capacity(struct distance *a, struct distance *b) {
    uint64_t d1, d2, c1, c2;
    double p1, p2;
    c1 = a->capacity;
    c2 = b->capacity;
    d1 = a->distance;
...
// By fee
int compare_distance_by_fee(struct distance *a, struct distance *b) {
    uint64_t d1, d2;
    double f1, f2;
    double p1, p2;
    f1 = a->weight;
...
// By timelock
int compare_distance_by_timelock(struct distance *a, struct distance *b) {
    uint64_t d1, d2, t1, t2;
    double p1, p2;
    t1 = a->timelock;
    t2 = b->timelock;
    d1 = a->distance;
...
int compare_dist(struct distance *a, struct distance *b) {
    if (COMPAREMETHOD == 0)
        return compare_distance(a, b);
    else if (COMPAREMETHOD == 1)
        return compare_distance_by_capacity(a, b);
    else if (COMPAREMETHOD == 2)
        return compare_distance_by_fee(a, b);
    else
        return compare_distance_by_timelock(a, b);
}
...
// empty heap
    while (heap_len(distance_heap[p]) != 0)
        heap_pop(distance_heap[p], compare_dist);

//  init, and node to distance[p] p:thread index
...
//  target nodes
    ...
    distance[p][target].capacity = 0;

    distance_heap[p] = heap_insert_or_update(distance_heap[p], &distance[p][target], compare_dist,
                                             is_key_equal);

    while (heap_len(distance_heap[p]) != 0) {

//      min heap
        d = heap_pop(distance_heap[p], compare_dist);
        best_node_id = d->node;
//      loop detection
        ...
        for (j = 0; j < array_len(best_node->open_edges); j++) {
            ...
            distance[p][from_node_id].capacity = channel->capacity;
            distance[p][from_node_id].fee = edge_fee;

            distance_heap[p] = heap_insert_or_update(distance_heap[p], &distance[p][from_node_id],
                                                     compare_dist,
                                                     is_key_equal);
        }
    }
...
struct array *
landmark(long source, long target, uint64_t amount, struct network *network, uint64_t current_time, long p,
         enum pathfind_error *error) {
    struct distance *d = NULL, to_node_dist;
    long i, best_node_id, j, from_node_id, curr;
    struct node *source_node, *best_node;
...
struct array *
yen(long source, long target, uint64_t amount, struct network *network, uint64_t current_time, long p,
    enum pathfind_error *error) {
    struct distance *d = NULL, to_node_dist;
    long i, best_node_id, j, from_node_id, curr;
    struct node *source_node, *best_node;
...
// empty heap
    while (heap_len(distance_heap[p]) != 0)
        heap_pop(distance_heap[p], compare_distance_by_timelock);
...
//  target nodes
    ...
    distance_heap[p] = heap_insert_or_update(distance_heap[p], &distance[p][target], compare_distance_by_timelock,
                                             is_key_equal);

    while (heap_len(distance_heap[p]) != 0) {

//      min heap
        d = heap_pop(distance_heap[p], compare_distance_by_timelock);
        best_node_id = d->node;
//      loop detection
        if (best_node_id == source) break;
...
struct array *
max_capacity_flow(long source, long target, uint64_t amount, struct network *network, uint64_t current_time, long p,
                  enum pathfind_error *error) {
    struct distance *d = NULL, to_node_dist;
    long i, best_node_id, j, from_node_id, curr;
    struct node *source_node, *best_node;
...
// empty heap
    while (heap_len(distance_heap[p]) != 0)
        heap_pop(distance_heap[p], compare_distance_by_capacity);
...
//  target nodes
    ...
    distance_heap[p] = heap_insert_or_update(distance_heap[p], &distance[p][target], compare_distance_by_capacity,
                                             is_key_equal);

    while (heap_len(distance_heap[p]) != 0) {

//      min heap
        d = heap_pop(distance_heap[p], compare_distance_by_capacity);
        best_node_id = d->node;
//      loop detection
        if (best_node_id == source) break;
...
struct array *
minimum_fee_flow(long source, long target, uint64_t amount, struct network *network, uint64_t current_time, long p,
                 enum pathfind_error *error) {
    struct distance *d = NULL, to_node_dist;
    long i, best_node_id, j, from_node_id, curr;
    struct node *source_node, *best_node;
...
// empty heap
    while (heap_len(distance_heap[p]) != 0)
        heap_pop(distance_heap[p], compare_distance_by_fee);
...
//  target nodes
    ...
    distance_heap[p] = heap_insert_or_update(distance_heap[p], &distance[p][target], compare_distance_by_fee,
                                             is_key_equal);

    while (heap_len(distance_heap[p]) != 0) {

//      min heap
        d = heap_pop(distance_heap[p], compare_distance_by_fee);
        best_node_id = d->node;
//      loop detection
        if (best_node_id == source) break;
