...
// 0 default(dijkstra) 1 max_flow_capacity 2 minimum_fee 3 yen 4 landmark
#define METHOD 1
...
int main(int argc, char *argv[]) {
    ...
    printf("INITIAL THREADS EXECUTION\n");
    clock_gettime(CLOCK_MONOTONIC, &start);
    // 0 default(dijkstra) 1 max_flow_capacity 2 minimum_fee 3 yen 4 landmark
    if (METHOD == 0)
        run_dijkstra_threads(network, payments, 0);
    else if (METHOD == 1)
        run_max_capacity_flow_threads(network, payments, 0);
    else if (METHOD == 2)
        run_minimum_fee_flow_threads(network, payments, 0);
    else if (METHOD == 3)
        run_yen_threads(network, payments, 0);
    else if (METHOD == 4)
        run_landmark_threads(network, payments, 0);
    clock_gettime(CLOCK_MONOTONIC, &finish);
    time_spent_thread = finish.tv_sec - start.tv_sec;
    printf("Time consumed by initial executions: %ld s\n", time_spent_thread);
